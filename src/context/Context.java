...
class RegisterManager {
    ...
    public String getReg(String rgType, String symbolType) {
        ...
        for (Map.Entry<String, BitSet> entry : rgMap.entrySet()) {
            if (rgType == "t" && (index == 0) && (Integer.parseInt(String.valueOf(entry.getKey().charAt(1))) > 6
...
public class Context {
    ...
    public void allocateMemory(int allocateMemory, String fid) // allocates memory to the stack for the current
                                                               // FunctionContext
    {
        if (allocateMemory < 4) // if we have a char just assign 4 bytes for it
            allocateMemory = 4;
        ...
        // System.out.printf("freeBytes: %d, requiredMemory: %d, allocMem: %d, functionStackSize: %d\n", freeBytes, requiredMemory, allocateMemory, functionStackSize);
        ...
        currentFunction.setcurrentSymbolOffset(currentFunction.getFreeBytes() - currentFunction.getStackSize());
        // System.out.printf("After: freeBytes: %d, functionStackSize: %d\n", currentFunction.getFreeBytes(), currentFunction.getStackSize());
    }
    ...
    public boolean symbolExistsInScope(String id) {
        ...
        return ((this._GlobalSymbolTable.containsKey(id)) || this._FunctionSymbolTable.containsKey(id)
                || this.getTopScope().getSymbol(id) != null); // does the symbol exist on the correct scope ?
    }
